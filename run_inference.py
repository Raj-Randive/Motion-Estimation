...
def main():
    ...
    os.makedirs(save_path.__str__(), exist_ok=True)
    ...
    test_files = []
    for ext in args.img_exts:
        for obj in os.scandir(data_dir.__str__()):
            if obj.is_file() and obj.name.endswith("1.{}".format(ext)):
                print(obj.path)
                # filepath = Path((os.join()))
                test_files.append(obj)
                pairname = obj.name.split(".")
                pairname[0] = pairname[0][:-1] + "2"
                img_pair = ".".join(pairname)
                print(os.path.join(data_dir.__str__(), img_pair))
                print(os.path.isfile(os.path.join(data_dir.__str__(), img_pair)))
                if os.path.isfile(os.path.join(data_dir.__str__(), img_pair)):
                    img_pairs.append(
                        [obj.path, os.path.join(data_dir.__str__(), img_pair)]
                    )
        # test_files = data_dir.files("*1.{}".format(ext))
        # for file in test_files:
        #     img_pair = file.parent / (file.stem[:-1] + "2.{}".format(ext))
    ...
    for img1_file, img2_file in tqdm(img_pairs):
        ...
        for suffix, flow_output in zip(["flow", "inv_flow"], output):
            img_name = img1_file.split("\\")[-1]
            filename = save_path.__str__() + "\{}{}".format(
                img_name.split(".")[0], suffix
            )
            print(filename)
